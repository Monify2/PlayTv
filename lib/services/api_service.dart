...
class ApiService {
  ...
  Future<Map<String, dynamic>> home() async =>
      Map<String, dynamic>.from(await call('home'));

  Future<List<PlayTitle>> titles({
    String? type,
    String? q,
    String? sort,
  }) async {
  ...
  Future<List<PlayTitle>> search(String q, {String? type}) async {
    final data = await call(
      'search',
      query: {'q': q, if (type != null) 'type': type},
    );
    return _titles(data);
  }

  Future<Map<String, dynamic>> title(String id) async =>
      Map<String, dynamic>.from(await call('titles/$id'));

  Future<List<PlayTitle>> watchlist() async {
    final data = await call('watchlist');
    final items = (data['items'] as List?) ?? const [];
    return items
        .where((e) => e is Map && e['title'] != null)
        .map((e) => PlayTitle.fromJson(Map<String, dynamic>.from(e['title'])))
        .toList();
  }

  Future<void> addWatchlist(String id) =>
      call('watchlist', method: 'POST', body: {'title_id': id});
  Future<void> removeWatchlist(String id) =>
      call('watchlist/$id', method: 'DELETE');

  Future<List<Progress>> progress() async {
    final data = await call('progress');
    return ((data['items'] as List?) ?? const [])
        .whereType<Map>()
        .map((e) => Progress.fromJson(Map<String, dynamic>.from(e)))
        .toList();
  }
  ...
  Future<List<dynamic>> plans() async =>
      (await call('plans'))['items'] ?? const [];
  Future<List<dynamic>> prices(String currency) async =>
      (await call('plans/prices', query: {'currency': currency}))['items'] ??
      const [];
  Future<Map<String, dynamic>> me() async =>
      Map<String, dynamic>.from(await call('me'));

  List<PlayTitle> _titles(dynamic data) =>
      ((data is Map ? data['items'] : null) as List? ?? const [])
          .whereType<Map>()
