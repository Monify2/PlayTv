...
class _DetailScreenState extends State<DetailScreen> {
  ...
  PlayTitle? get title {
    ...
    if (raw is Map) {
      return PlayTitle.fromJson(Map<String, dynamic>.from(raw));
    }
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Stack(
                fit: StackFit.expand,
                children: [
                  if (currentTitle.backdrop != null)
                    CachedNetworkImage(
                      imageUrl: currentTitle.backdrop!,
                      fit: BoxFit.cover,
                    ),
                  const DecoratedBox(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(currentTitle.releaseDate?.split('-').first ?? '—'),
                      const SizedBox(width: 12),
                      if (currentTitle.rating != null)
                        Text(currentTitle.rating!),
                      const SizedBox(width: 12),
                      const PlayTvIcon(
                        Icons.star_rounded,
                      ...
                      Text(currentTitle.score.toStringAsFixed(1)),
                    ],
                  ),
                  ...
                  const SizedBox(height: 18),
                  Text(
                    currentTitle.synopsis ?? 'No synopsis available.',
                    style: const TextStyle(
                  ...
                  if (seasons.isNotEmpty) ...[
                    ...
                    for (final s in seasons)
                      _SeasonCard(s: Map<String, dynamic>.from(s)),
                  ],
                  if (people.isNotEmpty) ...[
                    ...
                    SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          final p = Map<String, dynamic>.from(people[i]);
                          ...
                          return SizedBox(
                            ...
                            child: Column(
                              children: [
                                CircleAvatar(
                                  radius: 30,
                                  backgroundImage: p['profile_url'] != null
                                      ? NetworkImage('${p['profile_url']}')
                                      : null,
                                  child: p['profile_url'] == null
...
class _SeasonCard extends StatelessWidget {
  ...
  const _SeasonCard({required this.s});
      ...
      child: ExpansionTile(
        ...
        children: [
          for (final e in episodes)
            ListTile(
              leading: const PlayTvIcon(
                Icons.play_circle_outline_rounded,
                size: 29,
              ),
              title: Text(
                '${e['episode_number'] ?? ''}. ${e['name'] ?? 'Episode'}',
              ),
              subtitle: Text('${e['runtime'] ?? ''} min'),
              onTap: () {},
            ),
        ],
      ),
